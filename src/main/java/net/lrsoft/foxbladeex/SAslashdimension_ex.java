...
public class SAslashdimension_ex extends SpecialAttackBase implements IJustSpecialAttack, ISuperSpecialAttack {
    ...
    public void doSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if (entityId != 0) {
            ...
            if (tmp != null) {
                if (tmp.getDistanceToEntity(player) < 60.0f) target = tmp;
            }
        }
        ...
        if (target == null) {
            ...
            if (!player.worldObj.isRemote) {
                ...
                if (dim != null) {
                    ...
                    Vec3 offset = Vec3.createVectorHelper(player.posX, player.posY, player.posZ)
                    ...
                    if (movingobjectposition != null
                        ...
                        if (block != null && block.isCollidable()) {
                            Vec3 tmppos = Vec3.createVectorHelper(
            ...
            List<Entity> list = world
                .getEntitiesWithinAABBExcludingEntity(player, bb, ItemSlashBlade.AttackableSelector);

            if (!ItemSlashBlade.AttackableSelector.isEntityApplicable(target)) list.add(target);
    ...
    public void doJustSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if (entityId != 0) {
            Entity tmp = world.getEntityByID(entityId);
            if (tmp != null) {
                if (tmp.getDistanceToEntity(player) < 30.0f) target = tmp;
            }
        }
        ...
        if (target == null) {
            ...
            if (!player.worldObj.isRemote) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    EntityDrive entityDrive = new EntityDrive(
                        world,
                        player,
                        Math.min(1.0f, magicDamage / 3.0f),
                        false,
                        0);

                    float rotationYaw = 60 * i + (entityDrive.getRand()
                        .nextFloat() - 0.5f) * 60;
                    float rotationPitch = (entityDrive.getRand()
                        .nextFloat() - 0.5f) * 60;
            ...
            if (0 < level) {
                for (int i = 0; i < 5; i++) {
                    ...
                    float rotationYaw = target.rotationYaw + 60 * i
                        + (entityDrive.getRand()
                            .nextFloat() - 0.5f) * 60;
                    float rotationPitch = (entityDrive.getRand()
                        .nextFloat() - 0.5f) * 60;
